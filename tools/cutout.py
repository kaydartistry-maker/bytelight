...
the interpreter and the model through the environment:
    ...
    GIF_CUTOUT_PYTHON  interpreter with onnxruntime, numpy and pillow
    GIF_CUTOUT_MODEL   path to a u2net-style .onnx file
...
def apply_mask(
    ...
    if threshold is not None:
        ...
        # of half-transparent background colour around the subject.
...
def main() -> int:
    ...
    parser.add_argument("--model", default=os.environ.get("GIF_CUTOUT_MODEL", ""))
    args = parser.parse_args()

    if not args.model:
        raise SystemExit("cutout: set GIF_CUTOUT_MODEL or pass --model")
