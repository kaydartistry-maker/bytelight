...
import { splitResponse } from './utils.js';
import type { AgentService } from '../agent.js';
import { VoiceService } from '../voice.js';
import { createMessage, resolveRoutingThread, updateThreadActivity, setConfig } from '../db.js';
...
export class TelegramService {
  ...
  constructor(agentService: AgentService, registry: ConnectionRegistry, voiceService: VoiceService) {
    ...
    const token = getSecret('telegram_bot_token'); // DB → env; takes effect on restart/reconnect
  ...
  private async handleVoiceMessage(ctx: Context<Update.MessageUpdate>): Promise<void> {
    ...
    const isAllowedChat = chatId === telegramConfig.ownerChatId ||
                          (telegramConfig.groupChatId && chatId === telegramConfig.groupChatId);
  ...
  private async handleMessage(ctx: Context<Update.MessageUpdate>, overrideContent?: string): Promise<void> {
    ...
    // Auth: only owner's chat or configured group chat
    if (!telegramConfig.ownerChatId) {
      await ctx.reply('Send /start first to connect this chat.');
      return;
    }
  ...
  private async routeToAgent(
    ...
    // Route to the routing thread ("Home") — all owner messages converge here
    // (reference implementation port: the per-platform Telegram thread stops being created)
    const routedThread = resolveRoutingThread('telegram', this.registry);
    const threadId = routedThread.id;
    const threadName = routedThread.name;
    console.log(`[Telegram] Routed to routing thread: ${threadName} (${threadId})`);
    ...
    if (metadata.voiceTranscript && this.voiceService.canTTS && VoiceService.splitByCompanion(response).length > 0) {
      try {
   ...
   *
   * Multi-voice support: use emoji markers for auto-split:
   *   🔷 Companion A's line\n\n🔶 Companion B's line
   * Or pass voice param to force single voice: --voice companion-a or --voice companion-b
   */
  async sendVoiceToOwner(text: string, voice?: string): Promise<void> {
    const telegramConfig = getTelegramConfig();
    if (!telegramConfig.ownerChatId) {
    ...
    if (VoiceService.splitByCompanion(text).length === 0) {
      console.warn('[Telegram] Nothing speakable in voice text — falling back to text');
      await this.sendToOwner(text);
      return;
    }
    ...
    try {
      ...
      console.log(`[Telegram] Sent voice note (${audioBuffer.length} bytes)${voice ? ` voice=${voice}` : ''}`);
    } catch (error) {
  ...
  async sendGifToOwner(query: string, caption?: string): Promise<void> {
    ...
    try {
      const apiKey = getSecret('giphy_api_key');
      if (!apiKey) {
