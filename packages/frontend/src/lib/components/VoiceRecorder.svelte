<script lang="ts">
  import { send, getTranscriptionStatus, getTranscriptionText, getTranscriptionError, getTranscriptionProsody, getTranscriptionRecordingId, clearTranscription } from '$lib/stores/websocket.svelte';
  ...
  $effect(() => {
    // A stamped transcription (non-null recordingId) belongs to the live-call overlay's recording, not the composer mic — leave it untouched.
    if (getTranscriptionRecordingId() != null) return;
